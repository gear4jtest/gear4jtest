...
import static io.github.gear4jtest.core.model.refactor.OperationParamsInjector.*;

public final class TransformerIntrospector {

    private static final ConcurrentMap<Class<?>, Boolean> STATEFUL_CACHE = new ConcurrentHashMap<>();

    private TransformerIntrospector() {
        // utility
    }
    ...
    private static boolean detectStateful(Class<?> transformerClass) {
        return STATEFUL_CACHE.computeIfAbsent(transformerClass, TransformerIntrospector::scanForParameters);
    }
