...
public abstract class AbstractStation<I, O> implements Station<I, O> {
    ...
    @Override
    public final StationLog run(I input, ExecutionContext context) {
        ...
        var ctx = new DefaultStationExecutionContext(id, kind, context, record);
    ...
    protected O handleSkippedByCondition(I input,
        ...
        if (fallbackOperator != null) {
            try {
                result = fallbackOperator.transform(input, context, opContext);
                record.markSuccess(result);
            } catch (Exception e) {
     ...
     */
    protected void setUp(I input, ExecutionContext context, StationExecutionContext operationExecution) {
        // no-op par défaut
    }
     ...
     */
    protected void release(StationExecutionContext context, O result, List<Throwable> errors) {
        // no-op par défaut
    }
