...
/**
 * Station that calls another pipeline, either inline inside the current run or as a real nested run.
 */
public final class PipelineCallStation<IN, OUT> extends AbstractStation<IN, OUT> {
    ...
    private PipelineCallStation(
            String id,
            PipelineTarget<IN, OUT> target,
    ...
    public static <IN, OUT> PipelineCallStation<IN, OUT> inline(
            String id,
    ...
    public static <IN, OUT> PipelineCallStation<IN, OUT> nestedRun(
            String id,
            AssemblyLine<IN, OUT> childPipeline) {
        return PipelineCallStation.<IN, OUT>builder(id)
                .nestedRun()
