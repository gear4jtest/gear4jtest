...
//
//    static class StatefulAdder implements Transformer<Integer, Integer>, ConcurrencyAwareTransformer {
//        OperationParamsInjector.Parameter<Integer> delta = OperationParamsInjector.Parameter.ofDefault(1);
//
//        @Override public TransformerStatefulness statefulness() { return TransformerStatefulness.STATEFUL; }
//
//        @Override
//        public Integer transform(Integer input, ExecutionContext ctx, OperationExecutionContext op) {
...
//
//    @Test
//    void fullPipeline_shouldProcessIterator_thenConditional_thenFinalOp_withEvents_andPersistence() throws InterruptedException {
//        // ---------- EventBus ----------
//        BlockingQueue<Event> eventQueue = new LinkedBlockingQueue<>();
//
//        EventListener<Event> collector = eventQueue::add;
//        SimpleEventBus bus = new SimpleEventBus("bus", List.of(e -> true), List.of(collector));
//
...
//
//        ExecutionContext ctx = new ExecutionContext("pipe-main", eventManager, factory, execManager);
//
//        // ---------- Build pipeline ----------
//        // Iterator → add +1 → multiply *2 → conditional YES/NO → end
//
//        // Step 1 : Iterator on [1,2,3]
//        IteratorDefinition<Integer, Integer> iterator = ElementModelBuilders.iterate("iter")
...
//
//        long startedCount = eventQueue.stream().filter(e -> e instanceof OperationStartedEvent).count();
//        long completedCount = eventQueue.stream().filter(e -> e instanceof OperationCompletedEvent).count();
//
//        assertThat(startedCount).isGreaterThan(0);
//        assertThat(completedCount).isGreaterThan(0);
//
//        // ---------- Check persistence ----------
//        var opt = InMemoryPipelineExecutionRepository.INSTANCE.findById(res.getExecutionId());
//        assertThat(opt).isPresent();
//
//        PipelineExecution exec = opt.get();
//        assertThat(exec.getOperations().size()).isGreaterThanOrEqualTo(4); // iterator has multiple ops
//
...
//        @Override
//        public String transform(String input, ExecutionContext c, OperationExecutionContext op) {
//            return "DONE:" + input;
//        }
//    }
//
//    static class ConstantOperation extends AbstractOperationDefinition<List<Integer>, String> {
//        private final String result;
//        ConstantOperation(String id, String result) {
//            super(id, OperationKind.PROCESSING);
//            this.result = result;
//        }
//        @Override protected void setUp(List<Integer> x, ExecutionContext c, OperationExecutionContext oc) {}
//        @Override protected String doExecute(List<Integer> x, ExecutionContext c, OperationExecutionContext oc) { return result; }
//    }
//
