...
public class EnhancedDependencyAnalyzer {
    ...
    private final Pattern objectPattern = Pattern.compile("\\b([a-zA-Z_][a-zA-Z0-9_]*)\\.\\w+");
    ...
    private ResolvedDependency resolveDependency(String dependencyName) {
        ...
        if (mapping != null) {
            ...
            throw new IllegalArgumentException("Cannot infer type for dependency: " + dependencyName);
        }
    }
    
    private boolean isJavaKeyword(String word) {
        return Set.of("this", "super", "class", "int", "long", "double", "float", "boolean", 
                     "String", "Object", "System", "Math", "ctx", "input", "exec").contains(word);
    }
    
    private boolean isLocalVariable(String word) {
        return Set.of("ctx", "input", "exec", "e", "exception", "result").contains(word);
    }
