...
// LogManager.getLogger("AssemblyLineGenerator");
// private static final Pattern GENERIC_TYPE_PATTERN =
// Pattern.compile("(?<type>[\\w.]+)<(?<nestedClass>[\\w.<>]+)>");
//
// public static TypeSpec generateFlatAssemblyLines(InputStream is) {
// ByteArrayOutputStream baos = getByteArrayOutputStream(is);
//
// AssemblyLineValidator.FileParserReport report =
// AssemblyLineValidator.validate(new
// ByteArrayInputStream(baos.toByteArray()));
// if (report.getStatus() ==
// AssemblyLineValidator.FileParserReport.Status.FAILED) {
// throw new IllegalArgumentException(report.getThrowable());
// }
//
// try {
// AssemblyLine line = unmarshal(new InputSource(new
// ByteArrayInputStream(baos.toByteArray())), AssemblyLine.class);
...
// * @return the wanted jaxb object or null if an error was encountered while
// unmarshalling object.
// * @throws JAXBException
// */
// public static <T> T unmarshal(InputSource source, Class<T> type) throws
// JAXBException {
...
//
// ClassName pod =
// ClassName.get(AssemblyLineDefinition.class.getPackage().getName(),
// AssemblyLineDefinition.class.getSimpleName());
// ParameterizedTypeName ptn = ParameterizedTypeName.get(pod,
// mainLineSpecs.inType, mainLineSpecs.outType);
//
...
// }
//
// ClassName pod = ClassName.get(LineDefinition.class.getPackage().getName(),
// LineDefinition.class.getSimpleName());
...
// return new Specs(Arrays.asList(lineClass.build()), Arrays.asList(), inType,
// outType);
// }
//
...
// }
//
// private static Specs buildElement(Line.Operators.Operation operation) throws
...
// .addCode(codeBuilder.build())
// .build();
// return new Specs(Arrays.asList(), Arrays.asList(methodSpec),
// getTypeName(argumentsTypes[0].getTypeName()),
// getTypeName(argumentsTypes[1].getTypeName()));
// }
//
// private static Specs buildElement(Line.Operators.Container container) throws
// ClassNotFoundException {
...
//
// TypeName inType =
// list.stream().map(Specs::getInType).findFirst().orElse(getTypeName("java.lang.Void"));
//
// String containerReturnType =
// Optional.ofNullable(container.getReturns()).map(Line.Operators.Container.Returns::getJavaType).orElse("java.lang.Void");
// ClassName pod =
// ClassName.get(ContainerBaseDefinition.class.getPackage().getName(),
// ContainerBaseDefinition.class.getSimpleName());
// TypeName containerReturnTypeName = getTypeName(containerReturnType);
// ParameterizedTypeName ptn = ParameterizedTypeName.get(pod, inType,
// containerReturnTypeName);
//
// CodeBlock.Builder codeBuilder = CodeBlock.builder();
...
//
// return new
// Specs(list.stream().map(Specs::getTypeSpec).flatMap(List::stream).collect(Collectors.toList()),
// Arrays.asList(containerMethod), inType, containerReturnTypeName);
// }
//
...
// List<TypeName> nestedTypesName =
// Arrays.asList(nestedTypes).stream().map(AssemblyLineGenerator::getTypeName).collect(Collectors.toList());
// resultType = ParameterizedTypeName.get(ClassName.get(Class.forName(type)),
// nestedTypesName.stream().toArray(TypeName[]::new));
// } else {
...
//
// public Specs(List<TypeSpec> typeSpec, List<MethodSpec> methodSpec, TypeName
