...
class XmlOperationChainTranslatorTest {
    ...
    void should_compile_representative_generated_sources_against_current_core_api() throws IOException {
        ...
        List<String> samples = List.of(
                "/samples/good-assembly-line.xml",
                "/samples/sample-assembly-line.xml",
                "/samples/assembly-line-iterator.xml",
                "/samples/assembly-line-parallel-container.xml",
                "/samples/assembly-line-ifelse-container.xml",
                "/samples/assembly-line-signal.xml",
                "/samples/assembly-line-v2.xml",
                "/samples/assembly-line-database-persistence.xml");
        ...
        for (String sample : samples) {
            ...
            var compiledClasses = compiler.compile(result.className(), result.formattedSource().getBytes(StandardCharsets.UTF_8));

            // Then
            assertThat(compiledClasses)
                    .as("compiled classes for %s", sample)
                    .containsKey(result.className());
        }
    }
    ...
    void should_translate_compile_instantiate_inject_and_execute_generated_pipeline() throws Exception {
        ...
        ExecutionResult<?> result = engine().execute(pipeline, RunRequest.builder()
                .input("b")
