...
class DatabaseArtifactStoreTest {
    ...
    @Test
    void should_accept_simple_table_names() {
        // Given
        DataSource dataSource = mock(DataSource.class);

        // When / Then
        assertThatCode(() -> new DatabaseArtifactStore(dataSource, "gear4j_artifacts_1")).doesNotThrowAnyException();
        assertThatCode(() -> new DatabaseArtifactStore(dataSource, null)).doesNotThrowAnyException();
    }
}
