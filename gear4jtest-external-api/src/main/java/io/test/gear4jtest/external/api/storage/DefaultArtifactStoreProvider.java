...
/**
 * Construit un ArtifactStore (primaire + fallbacks) à partir d'une OperationChainConfig,
 * sans dépendre d'aucun backend concret (S3/SFTP/DB/FS). Les providers sont découverts via SPI.
 *
 * Props supportées côté config (exemples) :
 *  - mode.write = PRIMARY_ONLY | SYNC_ALL | ASYNC_FALLBACKS
 *  - mode.read  = PREFER_PRIMARY | FIRST_AVAILABLE
 *  - verifyOnRead = true|false
 *  - selfHealing  = true|false
 *  - fallback.1.type = S3
 *  - fallback.1.props.bucket = my-bucket
 *  - fallback.1.props.prefix = prod/
 *  - fallback.2.type = DATABASE
 *  - fallback.2.props.table = artifact_store
 */
public final class DefaultArtifactStoreProvider implements ArtifactStoreProvider {
     ...
     * @param ctx         Contexte facultatif (ex: lookup("datasource.default"), lookup("aws.s3.client"), …)
     * @param asyncExec   Executor pour l'async (fallbacks) et auto-healing (si activé).
     */
    public DefaultArtifactStoreProvider(ClassLoader classLoader,
    ...
    public ArtifactStore forConfig(OperationChainConfig cfg) {
        ...
        CompositeArtifactStore.WriteMode writeMode =
                parseWriteMode(props.getOrDefault("mode.write", "PRIMARY_ONLY"));
        CompositeArtifactStore.ReadMode readMode =
                parseReadMode(props.getOrDefault("mode.read", "PREFER_PRIMARY"));
        boolean verifyOnRead = isTrue(props.get("verifyOnRead"));
        boolean selfHealing  = isTrue(props.get("selfHealing"));

        return new CompositeArtifactStore(primary, fallbacks, writeMode, readMode, verifyOnRead, selfHealing, asyncExec);
    }
    ...
    private List<ArtifactStore> buildFallbacks(Map<String,String> props) {
        ...
        for (var e : props.entrySet()) {
            String k = e.getKey();
            if (!k.startsWith("fallback.")) continue;
        ...
        for (String idx : ordered) {
            ...
            Map<String,String> childProps = g.entrySet().stream()
                    ...
                    .collect(Collectors.toMap(
                            en -> en.getKey().substring("props.".length()),
                            Map.Entry::getValue
                    ));
    ...
    private static CompositeArtifactStore.WriteMode parseWriteMode(String s) {
        try { return CompositeArtifactStore.WriteMode.valueOf(s.toUpperCase(Locale.ROOT)); }
        catch (Exception e) { return CompositeArtifactStore.WriteMode.PRIMARY_ONLY; }
    }

    private static CompositeArtifactStore.ReadMode parseReadMode(String s) {
        try { return CompositeArtifactStore.ReadMode.valueOf(s.toUpperCase(Locale.ROOT)); }
        catch (Exception e) { return CompositeArtifactStore.ReadMode.PREFER_PRIMARY; }
    }
