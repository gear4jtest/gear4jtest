...
| F31 | Referential integrity across external stores | Mitigated | Polymorphic stores prevent one valid global FK. `ArtifactConsistencyChecker` detects orphan/missing references; durable cross-store cleanup remains operational work. |
...
| F35 | Missing `jreleaser.yml` | Closed in this lot | Root `jreleaser.yml` configures release-only signing, Central rules and `build/staging-deploy` without secrets. |
...
| F44 | Release path not executable | Closed in this lot, pending CI execution | `releaseCheck` stages, scans and invokes the autonomous consumer; `docs/releasing.md` documents credentials and dry run. |
...
| F46 | No external consumer test | Closed in this lot | `config/consumer-smoke` resolves staged coordinates, plugin marker and transitive API types without project dependencies. |
...
The source/configuration side is implemented: staged libraries, plugin markers, JReleaser configuration, dependency
scan and autonomous consumer are wired. The gate becomes effective only after `releaseCheck` and the JReleaser dry run
pass in CI with Gradle distribution access.
