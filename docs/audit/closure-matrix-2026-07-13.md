...
| F31 | Referential integrity across external stores | Mitigated for new manager publications in phase 9 | Polymorphic stores prevent one valid global FK. Durable staged metadata now precedes every manager artifact write, and `ArtifactPublicationReconciler` commits or abandons stale stages. `ArtifactConsistencyChecker` still detects metadata pointing to missing content. The generic SPI cannot enumerate legacy or out-of-band store-only artifacts, so retroactive reverse-orphan discovery remains future work. |
