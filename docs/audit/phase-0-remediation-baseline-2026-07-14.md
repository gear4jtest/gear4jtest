...
| A20 | Persistence shutdown timeout is not a real end-to-end bound | **Confirmed, narrowed after phase 5** | shutdown no longer waits on a monitor shared with normal operations, but its deadline still starts only after `closeAdmissionAndAwaitIdle()` returns; initial flushes and blocking per-buffer lock/JDBC acquisition still do not consume/check the remaining deadline. | 6 |
