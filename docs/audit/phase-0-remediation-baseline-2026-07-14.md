...
| A07 | Java domain objects do not enforce database/runtime invariants | **Partially closed in phase 9** | `OperationChainObject` and `OperationChainConfig` now enforce the V1 database lengths, required values, hashes, sizes and immutable properties. `FlowConfig` still accepts null policies and remains scheduled for later API hardening. | 10 |
