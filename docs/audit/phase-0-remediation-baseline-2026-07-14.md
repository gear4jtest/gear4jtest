...
| A04 | Generated compiler fallback/classloader semantics are inconsistent | **Closed in phase 11** | the default selects one backend at construction, javac source failures no longer retry through JDT, parent `URLClassLoader` entries augment the javac classpath, and JDT enables Java 17 release semantics. | Closed |
...
| A12 | Coverage gate protects only four classes | **Closed in phase 12, pending first connected calibration** | nine critical-class branch ratchets now cover concurrency, persistence, compiler, storage-provider and Spring defaults; six module-level line-coverage floors are versioned in `module-coverage-thresholds.json`. | Closed after connected CI calibration |
| A13 | Gradle plugin lacks TestKit/configuration-cache execution tests | **Closed in phase 11** | the task injects `FileSystemOperations`, TestKit executes the real generation task with strict configuration-cache validation and build-cache restoration, and CI runs the functional suite instead of checking `help`. | Closed |
...
| A16 | Dependency locking and cryptographic verification are optional in practice | **Deferred by explicit 1.0 decision** | Advanced locking and verification metadata are intentionally postponed. The 1.0 baseline keeps wrapper checksum, pinned actions, SCA and reproducible staging. | Revisit post-1.0 |
...
| A18 | Immutable generated content can be compiled repeatedly | **Closed in phase 11** | `AssemblyLineManager` shares a bounded 128-entry/16-MiB single-flight compiler cache between publication validation and runtime loading; a manager-level regression test proves the first load reuses validation bytecode. | Closed |
...
| A25 | JDBC/Testcontainers test dependencies are injected into unrelated modules | **Closed in phase 12** | database drivers and Testcontainers dependencies are now declared only for `gear4jtest-jdbc` and `gear4jtest-external-jdbc`. | Closed |
...
| A27 | Java 17 compatibility relies on build JDK instead of `--release` | **Closed in phase 12** | every module uses a Java 17 toolchain and every `JavaCompile` task uses `options.release = 17`; tests and JavaExec tasks use the Java 17 launcher. | Closed |
| A28 | Artifact reproducibility is not demonstrated | **Implemented in phase 12, pending connected proof** | dynamic `Created-By` was removed, all archive tasks disable timestamps and enforce stable order, and CI/release compare SHA-256 hashes from two clean staging builds. | Connected reproducibility job required |
...
./gradlew --no-daemon releaseCheck -PprojectVersion=1.0.0-rc1
