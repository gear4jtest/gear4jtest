# 0003 - AssemblyLine reference invalidation is a loader/cache concern
...
AssemblyLine versions are expected to be immutable.
...
Core pipeline-call traces record declared and resolved references when a `ResolvedAssemblyLineTarget` is used, so historical
