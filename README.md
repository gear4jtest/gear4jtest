...
It lets applications describe a pipeline as an `AssemblyLine`, compose it from typed stations, execute it through a runtime engine, and extend that runtime with persistence, events, side-compute, resource lookup, payload cloning and framework integrations.
...
- **Station**: an executable pipeline element. The core supports work stations, sequences, containers, if/else containers, iterators, signal stations and pipeline calls.
...
The Gradle wrapper must be complete in the working copy: `gradlew`, `gradle/wrapper/gradle-wrapper.jar` and `gradle/wrapper/gradle-wrapper.properties`.
...
The project is in active design and implementation. Prefer small, well-tested changes. When in doubt, keep the core simple and move optional integrations to dedicated modules.
